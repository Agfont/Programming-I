...
import static edu.ub.juanadearco.actors.Clau.ClausTrobats;
import static edu.ub.juanadearco.actors.Clau.NumberOfObjects;
...
public class Heroina extends AbstractActor {   
    ...
    public void addClau(Clau clau) {
        listaClaus[ClausTrobats] = clau;
    }
    
    public boolean haTrobatLesClaus() {
        if (ClausTrobats == NumberOfObjects-1) {
            return true;
        }
        else if (ClausTrobats >=1) {
            if ((listaClaus[ClausTrobats-1].nom).equals("Or")) {
                return true;
            }
            else {
                return false;
